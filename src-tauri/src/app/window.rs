...
pub fn position_cursor_window(
    ...
    window
        .eval("if (window.location.hash !== '#/cursor') window.location.hash = '/cursor';")
        .map_err(|error| error.to_string())?;
    ...
    let logical_window_size = ring_size.max(32.0) + 24.0;
    let physical_size = (logical_window_size * scale).ceil().max(1.0) as u32;
    let left = (x - physical_size as f64 / 2.0).round() as i32;
    let top = (y - physical_size as f64 / 2.0).round() as i32;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if current_size.width != physical_size || current_size.height != physical_size {
            window
                .set_size(tauri::PhysicalSize {
                    width: physical_size,
